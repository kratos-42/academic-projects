...
public class Agent implements VariableProvider {
  ...
  private static SnmpCommunityMIB mib;
  ...
  protected void addListenAddresses(MessageDispatcher md, List addresses) {
    for (Iterator it = addresses.iterator(); it.hasNext();) {
      String gen = (String)it.next();
      System.out.println(gen);
      String[] sep = gen.split("/");
      sep[1] = String.valueOf(port);
      gen = sep[0]+"/"+sep[1];

      Address address = GenericAddress.parse(gen);
      System.out.println(gen);
   ...
   */
  protected void registerMIBs() throws IOException, ParseException
  {
    if (modules == null) {
  ...
  private static void defaultMain(String args[]) throws IOException{
