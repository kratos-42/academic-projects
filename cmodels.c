...
void create_plane(char* finlename, float comprimento, float largura, int camadasH, int camadasV){

  //pontos iniciais
  FILÈ* file = fopen(filename, "w");
  ...
  if (strcmp("plano", model) == 0){
    create_plane((char*)(argv[2]), atof(argv[3]), atoi(argv[4]), atoi(argv[5]), atoi(argv[6]));
    }
  }
